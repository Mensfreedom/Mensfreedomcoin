...
#else
    const char* pszModule = "mensfreedomcoin";
#endif
    if (pex)
        return strprintf(
...
#else
    /* Linux, Mac OS X, *BSD and so on: ~/.mensfreedomcoin */
    char* pszHome = getenv("HOME");
    if((pszHome == NULL) || (strlen(pszHome) == 0))
      path = fs::path("/.mensfreedomcoin");
    else
      path = fs::path(pszHome) / ".mensfreedomcoin";
#endif
...
boost::filesystem::path GetConfigFile() {
    ...
    else pathConfigFile = fs::path("mensfreedomcoin.conf");
...
void ReadConfigFile(map<string, string>& mapSettingsRet, map<string, vector<string> >& mapMultiSettingsRet) {
    ...
    if (!streamConfig.good())
        return; // No mensfreedomcoin.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override mensfreedomcoin.conf
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct.  If your clock is wrong Mensfreedomcoin will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("Mensfreedomcoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
